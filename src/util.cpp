...
int nAnonymizeDigitalPriceAmount = 1000;
...
#else
    const char* pszModule = "digitalprice";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DigitalPrice
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DigitalPrice
    // Mac: ~/Library/Application Support/DigitalPrice
    // Unix: ~/.digitalprice
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DPrice";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "digitalprice.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "digitalpriced.pid"));
