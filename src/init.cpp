...
void Shutdown()
{
    ...
    RenameThread("digitalprice-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: digitalprice.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: digitalpriced.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizedigitalpriceamount=<n> " + _("Keep N DigitalPrice anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DigitalPrice is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DigitalPrice is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DigitalPrice version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DigitalPrice server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DigitalPrice") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DigitalPrice to complete") << "\n";
    ...
    nAnonymizeDigitalPriceAmount = GetArg("-anonymizedigitalpriceamount", 0);
    if(nAnonymizeDigitalPriceAmount > 999999) nAnonymizeDigitalPriceAmount = 999999;
    if(nAnonymizeDigitalPriceAmount < 2) nAnonymizeDigitalPriceAmount = 2;
    ...
    LogPrintf("Anonymize DigitalPrice Amount %d\n", nAnonymizeDigitalPriceAmount);
