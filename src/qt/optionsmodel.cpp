...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDigitalPriceAmount"))
        settings.setValue("nAnonymizeDigitalPriceAmount", 1000);
    nAnonymizeDigitalPriceAmount = settings.value("nAnonymizeDigitalPriceAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDigitalPriceAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDigitalPriceAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDigitalPriceAmount:
            return QVariant(nAnonymizeDigitalPriceAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDigitalPriceAmount:
            nAnonymizeDigitalPriceAmount = value.toInt();
            settings.setValue("nAnonymizeDigitalPriceAmount", nAnonymizeDigitalPriceAmount);
            emit AnonymizeDigitalPriceAmountChanged(nAnonymizeDigitalPriceAmount);
            break;
        default:
            break;
        }
