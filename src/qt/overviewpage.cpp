...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDigitalPriceAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDigitalPriceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDigitalPriceAmount = strAnonymizeDigitalPriceAmount.remove(strAnonymizeDigitalPriceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalPriceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDigitalPriceAmount*COIN) nMaxToAnonymize = nAnonymizeDigitalPriceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDigitalPriceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDigitalPriceAmount));
        strAnonymizeDigitalPriceAmount = strAnonymizeDigitalPriceAmount.remove(strAnonymizeDigitalPriceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigitalPriceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDigitalPriceAmount == 0){
