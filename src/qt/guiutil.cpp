...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("digitalprice"))
        return false;
...
{
    // Convert digitalprice:// to digitalprice:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("digitalprice://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "digitalprice:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DigitalPrice.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DigitalPrice.lnk
...
{
    return GetAutostartDir() / "digitalprice.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a digitalprice.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DigitalPrice\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the digitalprice app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add digitalprice app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DigitalPrice-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  digitalprice-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DigitalPrice-Qt"));
